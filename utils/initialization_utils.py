...
def initialize_experiment(params, file_name):
    ...
    if file_name == "test_auc.py":
        params.test_exp_dir = os.path.join(
            params.exp_dir, f"test_{params.dataset}_{params.constrained_neg_prob}"
        )
        if not os.path.exists(params.test_exp_dir):
            os.makedirs(params.test_exp_dir)
        file_handler = logging.FileHandler(
            os.path.join(params.test_exp_dir, f"log_test.txt")
        )
    else:
        file_handler = logging.FileHandler(
            os.path.join(params.exp_dir, "log_train.txt")
        )
...
def initialize_model(params, model, load_model=False):
    ...
    if load_model and os.path.exists(model_path):
        ...
        relation2id_path = os.path.join(
            params.data_dir, f"data/{params.dataset}/relation2id.json"
        )
